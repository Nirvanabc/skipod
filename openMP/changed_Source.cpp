...
{
        int size, block_size, threds_num;
	// cout << "enter matrix size, block_size, threds_num\n";
	cin >> size >> block_size >> threds_num;
	omp_set_num_threads(threds_num);
	...
	for (int it = 0; it < 1; it++)
	{
		...
		t2 = high_resolution_clock::now();
		duration = std::chrono::duration_cast<std::chrono::microseconds>(t2 - t1).count();
		cout << "Time_elapsed: " << static_cast <float> (duration) / 1000000.0f << endl;
		// cout << static_cast <float> (duration) / 1000000.0f << endl;
	}
	return 0;
}
...
#pragma omp parallel shared(A, B, C, size, chunk) private(i, j, k, jj, kk, tid, tmp)
	{
	  ...
	  // omp_set_num_threads(40);
		...
		if (tid == 0)
		{
		  cout << "Number_of_threads: " << omp_get_num_threads() << " matrix_size: " << size << " ";
		}
		#pragma omp for schedule (static, chunk)
